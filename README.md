...
1. Set `file_path` (single file) or `folder_path` (batch mode) — leave the other empty.
2. Set `output_folder` for processed outputs and plots.
3. Choose algorithms for each stage (`cosmic_removal_tool_name`, `baseline_correction_tool_name`, `smoothing_tool_name`, `normalization_tool_name`) and toggle stages on/off with the `ENABLE_*` flags.
4. (Optional) Set `SILICON_REFERENCE_FILE` and `ENABLE_WAVENUMBER_CALIBRATION = True` to calibrate the x-axis against a silicon reference before processing.
5. Run:

```bash
python Raman_Pre_processing.py
```
